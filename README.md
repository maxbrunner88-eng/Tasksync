...
`/loop` lebt in der Session — endet der Container, endet der Loop. Für einen Abgleich, der wirklich jeden Tag läuft, zusätzlich eine **Routine** anlegen (täglich 07:00, startet eine frische Session mit `/tasksync`). Beides parallel ist unschädlich: der Sync ist idempotent, ein zweiter Lauf direkt hinter dem ersten meldet null Schreibvorgänge.
